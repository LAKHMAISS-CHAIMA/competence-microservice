...
function CompetenceForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post("http://localhost:8000/competences", {
        code,
        nom,
        sousCompetence: sousCompetences.map(sc => ({
          nom: sc.nom,
          statut: "non validée"
        })),
      });
  ...
  const startEditSousCompetence = (index) => {
    setEditingIndex(index);
    setEditingValue(sousCompetences[index].nom);
  };

  const cancelEditSousCompetence = () => {
    setEditingIndex(null);
    setEditingValue("");
  };

  const validateEditSousCompetence = (index) => {
    handleSousCompetenceChange(index, editingValue);
    setEditingIndex(null);
    setEditingValue("");
  };
  ...
  return (
    <div className="bg-gradient-to-br from-violet-100 via-pink-50 to-rose-100 shadow-lg p-4 sm:p-8 max-w-2xl mx-auto rounded-2xl mt-8 border border-violet-100">
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <h2 className="text-2xl md:text-3xl font-extrabold text-violet-700 mb-2 text-center">Ajouter une compétence</h2>
                ...
                {editingIndex === index ? (
                  <>
                    <input
                      ...
                      autoFocus
                    />
                    <button type="button" onClick={() => validateEditSousCompetence(index)} className="p-2 rounded-full hover:bg-green-100 group transition" title="Valider">
                    ...
                    <button type="button" onClick={cancelEditSousCompetence} className="p-2 rounded-full hover:bg-gray-100 group transition" title="Annuler">
                  ...
                  <>
                    ...
                    <button type="button" onClick={() => startEditSousCompetence(index)} className="p-2 rounded-full hover:bg-fuchsia-100 group transition" title="Éditer">
        ...
        >
          ...
          ></span>
          <span className="relative text-base font-semibold">Ajouter la compétence</span>
        </button>
      </form>
    </div>
  );
